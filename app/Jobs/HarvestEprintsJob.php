...
class HarvestEprintsJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $url = rtrim($gatewayUrl, '/') . '/eprints';
        $token = config('services.emsacode.token', 'mock-token-123') ?: 'mock-token-123';

        Log::info("Queue Job starting: Harvesting EPrints via Gateway: {$url}");
        ...
        Log::info("Queue Job success: Harvested {$count} EPrints records.");
    }
    ...
    public function failed(\Throwable $exception): void
    {
        ...
        HarvesterService::logFailure('eprints', $url, "Queue Job Failed after all retries. Reason: " . $exception->getMessage());
    }
}
